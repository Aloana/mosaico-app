...
import { TIME_RANGE_FILTERS, SKILL_AREA_FILTERS, RECORD_SOURCE_FILTERS } from '../../constants/filterOptions';
...
// --- TELA PRINCIPAL ---
const TeacherReportScreen = ({ navigation }) => {
    ...
    return (
        <ScreenWrapper barBackgroundColor="#fff">
            <Header title="Relatórios da Turma" onBackPress={() => navigation.goBack()}>
            ...
            <ScrollView contentContainerStyle={styles.content}>
                <View style={styles.filtersContainer}>
                    ...
                    <TouchableOpacity style={styles.filterButton} onPress={() => setSkillModalVisible(true)}>
                        <Text style={styles.filterText}>{getLabel(SKILL_AREA_FILTERS, skill)}</Text>
                        <Icon name="chevron-down" size={12} color="#334155" />
                    </TouchableOpacity>
                </View>

                <View style={styles.card}>
                    <Text style={styles.cardTitle}><Icon name="check-circle" color="#4CAF50" solid /> Conclusão de Atividades da Turma</Text>
                    <View style={styles.chartContainer}>
                        <ChartBar label="Sem 1" percentage={75} />
                        <ChartBar label="Sem 2" percentage={80} />
                        <ChartBar label="Sem 3" percentage={90} />
                        <ChartBar label="Sem 4" percentage={85} />
                    </View>
                </View>

                <Text style={styles.sectionTitle}>Alunos que Requerem Atenção</Text>
                
                {/* Aqui entraria uma lista de alunos, por exemplo */}
                <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('StudentDetails')}>
                    <Text style={styles.cardContent}>Lucas Souza - Dificuldade em Coordenação Motora</Text>
                </TouchableOpacity>
